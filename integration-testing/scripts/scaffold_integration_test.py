...
def generate_jbang_launcher(module_info: Dict, bean: Dict) -> str:
...
def create_integration_test(module_path: str, complexity: str = 'simple', force: bool = False, ai_validation: bool = True, ai_mode: str = None):
    ...
    bean = get_config_template(complexity)
    
    # Apply AI validation options
    if not ai_validation:
        # Remove AI validation section
        bean.pop('aiValidation', None)
    elif ai_mode:
        # Override validation mode if specified
        if 'aiValidation' in bean:
            bean['aiValidation']['validationMode'] = ai_mode
    ...
    launcher_content = generate_jbang_launcher(module_info, bean)
    with launcher_file.open("w") as f:
        f.write(launcher_content)
    
    ai_enabled = 'aiValidation' in bean
    ai_mode = bean.get('aiValidation', {}).get('validationMode', 'N/A') if ai_enabled else 'disabled'
    
    print(f"✅ Created integration test for {module_path}")
    print(f"   📄 {config_file}")
    print(f"   ☕ {launcher_file} (using centralized utilities)")
    print(f"   🎯 Complexity: {complexity}")
    print(f"   ⏱️  Timeout: {bean['timeoutSec']}s")
    print(f"   🔍 Success patterns: {len(bean['successRegex'])}")
